...
/*   Updated: 2023/07/23 23:44:19 by smclacke      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	extra_hooking(t_fractol *data)
{
	if (mlx_is_key_down(data->mlx, MLX_KEY_J))
	{
		...
		data->julia[X] = data->x[LT] + ((double)data->mouse[X] / data->win_width)
			* (data->x[RT] - data->x[LT]);
		data->julia[Y] = data->y[UP] + ((double)data->mouse[Y] / data->win_height)
			* (data->y[DN] - data->y[UP]);
		fractal(data);
	}
